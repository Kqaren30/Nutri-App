...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-white px-2" data-testid="login-page">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-10 flex flex-col items-center border border-gray-100" data-testid="login-container">
        <div className="flex items-center gap-2 mb-8" data-testid="login-header">
          ...
          <span className="font-extrabold text-2xl text-green-600 tracking-tight" data-testid="login-title">NutriApp</span>
        </div>
        <h2 className="text-4xl font-extrabold mb-2 text-center text-gray-800" data-testid="login-welcome">Bienvenido</h2>
        ...
        <form className="space-y-6 w-full" onSubmit={handleSubmit} autoComplete="on" data-testid="login-form">
          ...
          {error && <p className="text-red-500 text-center" data-testid="login-error-message">{error}</p>}
          <button type="submit" className="w-full bg-green-500 text-white py-3 rounded-xl font-bold text-lg hover:bg-green-600 transition shadow focus:outline-none focus:ring-2 focus:ring-green-300" disabled={loading} data-testid="login-submit-button">
            {loading ? 'Ingresando...' : 'Iniciar sesión'}
          </button>
        </form>
        <p className="mt-6 text-center text-sm text-gray-600" data-testid="login-register-link">
