...
export default function DishesClientList({ dishes }: { dishes: Dish[] }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8" data-testid="dishes-list">
      {dishList.map(dish => (
        <div key={dish.id} className="bg-white rounded-2xl shadow-lg p-6 flex flex-col hover:shadow-xl transition group border border-gray-100" data-testid={`dish-card-${dish.id}`}>
          <div className="relative mb-4" data-testid="dish-image-container">
            {dish.imageUrl ? (
            ...
            <span className="absolute top-2 right-2 bg-green-400 text-white text-xs font-bold px-4 py-1 rounded-full shadow tracking-wide uppercase" data-testid="dish-prep-time">
